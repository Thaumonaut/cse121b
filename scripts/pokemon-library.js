...
export function SelectTeam(pokemonList, size = 6) {
  ...
  let pokemonTeam = Array.from(indexes).map(
...
export async function SelectMoves(pokemon) {
  ...
  const movesList = randomList.map(function (ID) {
    const move = moves[ID].move;
    return {
...
export async function MovesType(moves) {
    const movesInfo = await moves.map(async (move) => {
        const response = await fetch(move.url)
        const data = await response.json();
        console.log(data)
        return data.type.name;
    });
    return await Promise.all(movesInfo)
}
